...
const App = ({isAdmin}) => {
  ...
  const [ modalOpenE, setModalOpenA ] = useState(false);
  ...
  const handleApprove = () => {
    post(`${process.env.REACT_APP_API_URL}/vip/updatePay`, header, { lineId: id })
    .then((resp)=>{
      if (resp.data.status === 'success') {
        get(`${process.env.REACT_APP_API_URL}/vip/toBeCharge`, header)
        .then((resp)=>{
  ...
  const handleRemove = () => {
    post(`${process.env.REACT_APP_API_URL}/vip/removeVip`, header, { lineId: id })
    .then((resp)=>{
      if (resp.data.status === 'success') {
        get(`${process.env.REACT_APP_API_URL}/vip/toBeCharge`, header)
        .then((resp)=>{
  ...
  return (
    <>
      <Modal title="調整 VIP 會員方案" open={modalOpenE} onCancel={handleCancelA}
        footer={[
          ...
          <List.Item actions={[<Actions lineId={item.lineUserId}/>]} style={{ padding: 12 }}>
            <List.Item.Meta
              ...
              description={
                <>
                  <p>{item.vipPlan}</p>
                  <p style={{ color: paydayColor(item.due) }}>期限{item.due}</p>
                </>
              }
            />
          </List.Item>
